                ...
                <table id="basic-datatable" class="table m-10 table-bordered" data-filter-control="true" data-toggle="table"
                    ...
                    <thead class="table-light">
                        <tr>
                            ...
                            <th data-field="product_id" data-filter-control="input" data-sortable="true">
                                {{ __('main.code') }}</th>
                            <th data-field="product_name" data-filter-control="input" data-sortable="true">
                                {{ __('dashboard.name') }}</th>
                            <th data-field="category" data-filter-control="select" data-sortable="true">
                                {{ __('main.Category') }}</th>
                            <th data-field="active" data-filter-control="select" data-sortable="true">
                    ...
                    <tbody>
                        @foreach ($products as $item)
                            <tr>
                                <td>
                                    <img src="{{ $item->img }}" id="gmaeImage{{ $item->id }}" class="image-upload"
                                ...
                                <td>
                                    <img src="{{ $item->img_mini }}" id="gmaeImage2{{ $item->id }}"
                                        class="image-upload" style="max-height: 40px;cursor: pointer;">
                                    ...
                                    <div class="custom-control custom-checkbox">
                                        ...
                                        @if ($item->active == 1)
                                            <label class="custom-control-label"
                                                for="customCheck{{ $item->id }}">Enable</label>
                                        @else
                                            <label class="custom-control-label"
                                                for="customCheck{{ $item->id }}">Disable</label>
                                        @endif
                                ...
                                <td>
                                    <div class="d-flex">
                                        ...
                                        <button type="button" class="btn btn-primary btn-sm waves-effect waves-light ml-2"
                                            onclick="editOrderTop('{{ $item->id }}')"><i
                                ...
                                <td>
                                    <div class="d-flex">
                                        ...
                                        <button type="button" class="btn btn-primary btn-sm"
                                            onclick='openEditModal(@json($item))' data-toggle="modal"
                                            data-target="#editProductModal">
                                            <i class="bx bx-edit-alt"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm ml-2"
                                            onclick="confirmDelete('{{ route('products.destroy', $item->id) }}')">
                                            <i class="bx bx-trash"></i> {{-- เพิ่มไอคอนถังขยะ ถ้าต้องการ --}}
                                        </button>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="file" class="form-control-file" name="img" id="img"
                                    accept="image/*" onchange="previewImage(this, 'previewImg')">
                                <img id="previewImg" src="#" alt="Preview"
                                    style="width: 100%; max-height: 250px; margin-top: 10px; display: none; object-fit: contain;">
                            </div>

                            <div class="col-md-6">
                                <label for="img_mini">รูปขนาดย่อ</label>
                                <input type="file" class="form-control-file" name="img_mini" id="img_mini"
                                    accept="image/*" onchange="previewImage(this, 'previewMini')">
                                <img id="previewMini" src="#" alt="Preview"
                                    style="width: 100%; max-height: 150px; margin-top: 10px; display: none; object-fit: contain;">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="file" class="form-control-file" name="img" id="edit_img"
                                    accept="image/*" onchange="previewImage(this, 'previewEditImg')">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="file" class="form-control-file" name="img_mini" id="edit_img_mini"
                                    accept="image/*" onchange="previewImage(this, 'previewEditMini')">
                                <img id="previewEditMini" src="#"
                                    style="width: 100%; max-height: 150px; margin-top: 10px; display: none; object-fit: contain;">
                            </div>
                        </div>
                    </div>
...
@section('scripts')
    ...
    <script>
        ...
        $('#imgupload').change(function(e) {
            ...
            if (fname !== "png" && fname !== "jpg" && fname !== "jpeg" && fname !== "webp" && fname !== "gif" &&
                fname !== "svg") {
                Swal.fire({
                    position: 'top-end',
                    icon: 'error',
        ...
        function confirmDelete(deleteUrl) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // สร้างฟอร์มแบบไดนามิกเพื่อส่งคำขอแบบ DELETE
                    const form = document.createElement('form');
                    form.action = deleteUrl;
                    form.method = 'POST';
                    form.style.display = 'none';

                    // เพิ่ม CSRF token เพื่อความปลอดภัย
                    ...
                    // เพิ่ม method spoofing สำหรับ Laravel
