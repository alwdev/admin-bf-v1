...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0 font-size-18">จัดการเกม</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">เพจ</a></li>
                        <li class="breadcrumb-item active">จัดการเกม</li>
                    </ol>
                </div>
            ...
            <div class="card-body">
                ...
                <table id="table" class="table m-10 table-bordered" data-filter-control="true" data-toggle="table"
                    ...
                    <thead>
                        <tr>
                            <th></th>
                            <th data-field="name" data-filter-control="input" data-sortable="true">ชื่อ</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($gamelist as $item)
                            <tr>
                                <td onclick="chooseImage(this,{{ $item->id }},'{{ $item->productId }}')">
                                    <img src="@if(is_null($item->bannerUrl)) - @else @if($item->bannerUrl == "") - @else {{ $item->bannerUrl }} @endif @endif" id="gmaeImage{{ $item->id }}" onclick="chooseImage(this,{{ $item->id }},'{{ $item->productId }}')"   class="image-upload" style="max-height: 250px;cursor: pointer;">
                                </td>
                                <td>{{ $item->gameName }}</td>
                                <td>
    ...
    <form action="#" method="POST" id="img-upload" enctype="multipart/form-data" >
        @csrf
            ...
            <input type="hidden" id="game_id" name="game_id" value="" style="display:none"/>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        url: '{{ route('gamelist.updategamestatus') }}',
                        data: {
                            id: id,
                            checked: active
                        },
        ...
        $.ajax({
            ...
            success: (response) => {
                if (response) {
                    ...
                    $('#gmaeImage'+ $('#game_id').val()).attr('src',imgname);
