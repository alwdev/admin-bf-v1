                ...
                <div class="d-flex justify-content-between mb-3 align-items-end">
                    ...
                    <form class="form-inline" method="GET" action="{{ route('sbo.providers.index') }}">
                        ...
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createModal"><i
                                class="bx bx-plus"></i> เพิ่ม Provider</button>
                    </form>
                </div>
                            ...
                            <tr>
                                <td>
                                    <img src="{{ $item->img ?: asset('images/logo.png') }}"
                                        id="providerImage{{ $item->id }}"
                                        onclick="chooseProviderImage({{ $item->id }})"
                                ...
                                <td>
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input"
                                            id="providerActive{{ $item->id }}"
                                            onchange="toggleProvider({{ $item->id }}, this.checked)"
                                            {{ $item->active ? 'checked' : '' }}>
                                        <label class="custom-control-label"
                                            for="providerActive{{ $item->id }}">{{ $item->active ? 'Enable' : 'Disable' }}</label>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn btn-warning btn-sm"
                                        onclick="openProviderEdit({{ $item->id }})"><i
                                            class="bx bx-edit-alt"></i></button>
                                    <form action="{{ route('sbo.providers.destroy', $item->id) }}" method="POST"
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('sbo.providers.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="createProviderActive" name="active"
                                checked>
            ...
            <div class="modal-content">
                ...
                <form id="editProviderForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="edit_provider_active"
                                name="active">
